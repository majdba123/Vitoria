...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    function populateSubs(catId) {
        ...
        $('f-subcategory').innerHTML = '<option value="">كل التصنيفات الفرعية</option>' + subs.map(s => `<option value="${s.id}">${esc(s.name)}</option>`).join('');
    }

    $('f-category-type').addEventListener('change', function() { ct = this.value; cId = ''; sId = ''; populateCategories(); populateSubs(''); });
    $('f-category').addEventListener('change', function() { cId = this.value; populateSubs(this.value); sId = ''; });
    ...
    window.goP = function(targetPage) { page = targetPage; load(); window.scrollTo({ top: 0, behavior: 'smooth' }); };
    ...
    $('btn-clear').addEventListener('click', () => { $('f-category-type').value = ''; $('f-category').value = ''; $('f-subcategory').innerHTML = '<option value="">كل التصنيفات الفرعية</option>'; $('f-discount').value = ''; ct = ''; cId = ''; sId = ''; dFilter = ''; page = 1; populateCategories(); load(); });
