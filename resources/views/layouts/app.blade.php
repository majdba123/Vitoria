...
>
        ...
        <div class="workspace-shell pb-10 pt-6">
                ...
                <div class="col-span-1 sm:col-span-2 lg:col-span-2">
                    ...
                    <div class="mt-6 flex gap-3">
                        @if(!empty($footerSettings->facebook_url))
                            <a href="{{ $footerSettings->facebook_url }}" target="_blank" rel="noopener noreferrer" class="footer-social-link" aria-label="Facebook">
                                <svg class="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                            </a>
                        @endif
                        @if(!empty($footerSettings->instagram_url))
                            <a href="{{ $footerSettings->instagram_url }}" target="_blank" rel="noopener noreferrer" class="footer-social-link" aria-label="Instagram">
                        ...
                        @if(!empty($footerSettings->twitter_url))
                            <a href="{{ $footerSettings->twitter_url }}" target="_blank" rel="noopener noreferrer" class="footer-social-link" aria-label="Twitter">
            ...
            <div class="mt-10 flex flex-col items-center justify-between gap-4 border-t border-gray-200/70 pt-8 sm:flex-row dark:border-gray-800/70">
                ...
                <div class="flex gap-6 text-xs text-gray-400 dark:text-gray-500">
                    <a href="{{ route('pages.show', ['slug' => 'privacy-policy']) }}" class="hover:text-gray-600 dark:hover:text-gray-300">{{ __('footer.privacy') }}</a>
                    <a href="{{ route('pages.show', ['slug' => 'terms-of-service']) }}" class="hover:text-gray-600 dark:hover:text-gray-300">{{ __('footer.terms') }}</a>
                </div>
            </div>
            </div>
        </div>
    ...
    (function(){
        ...
        function showStep(nextStep) {
            ...
            nextBtn.textContent = isTimezone ? @js(__('startup.finish')) : @js(__('startup.next'));
            titleEl.textContent = isTimezone ? @js(__('startup.timezone_title')) : @js(__('startup.modal_title'));
        ...
        nextBtn?.addEventListener('click', async function () {
            ...
            try {
                ...
                showError(error.response?.data?.message || @js(__('startup.save_failed')));
            } finally {
