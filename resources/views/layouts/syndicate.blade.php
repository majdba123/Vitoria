...
<head>
    ...
    <title>@yield('title', 'Syndicate Dashboard - SyriaZone')</title>
...
<body class="min-h-screen bg-gray-50 font-sans text-gray-900 antialiased dark:bg-gray-950 dark:text-gray-100">
    <div id="syndicate-app" class="hidden">
        ...
        <aside id="syndicate-sidebar" class="fixed inset-y-0 left-0 z-50 flex w-72 -translate-x-full flex-col bg-gray-900 transition-transform duration-300 lg:translate-x-0">
            <div class="flex h-14 items-center gap-3 border-b border-white/10 px-6">
                <a href="{{ route('syndicate.dashboard') }}" class="text-xl font-bold text-white">Syria<span class="text-emerald-400">Zone</span></a>
                <span class="rounded-md bg-emerald-500/15 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-emerald-400">Syndicate</span>
            ...
            @php($links = [
                ['route' => 'syndicate.dashboard', 'label' => 'Overview'],
                ['route' => 'syndicate.categories', 'label' => 'Categories'],
                ['route' => 'syndicate.vendors', 'label' => 'Merchants'],
                ['route' => 'syndicate.products', 'label' => 'Products'],
                ['route' => 'syndicate.podcasts', 'label' => 'Podcasts'],
                ['route' => 'syndicate.orders', 'label' => 'Orders'],
                ['route' => 'syndicate.sales', 'label' => 'Sales'],
                ['route' => 'syndicate.reports', 'label' => 'Reports'],
            ])
            <nav class="flex-1 overflow-y-auto px-4 py-5">
                <p class="mb-2 px-3 text-[10px] font-bold uppercase tracking-widest text-gray-500">Workspace</p>
        ...
        <div class="lg:pl-72">
            <header class="sticky top-0 z-30 border-b border-gray-200 bg-white/95 backdrop-blur dark:border-gray-800 dark:bg-gray-900/95">
                <div class="flex h-14 items-center gap-4 px-4 sm:px-6 lg:px-8">
                    ...
                    <h1 class="flex-1 text-base font-semibold text-gray-900 dark:text-white">@yield('page-title', 'Syndicate')</h1>
                    <span id="syndicate-type-badge" class="badge badge-brand"></span>
                    <button onclick="syndicateLogout()" class="btn-secondary btn-sm">Sign out</button>
                </div>
            </header>
    ...
    <div id="syndicate-loading" class="flex min-h-screen items-center justify-center">
        <div class="text-center"><div class="mx-auto h-10 w-10 animate-spin rounded-full border-4 border-gray-200 border-t-emerald-500"></div><p class="mt-4 text-sm text-gray-500">Verifying syndicate access...</p></div>
    </div>
    <script>
        ...
        document.addEventListener('DOMContentLoaded', async function () {
            ...
            try {
                ...
                document.getElementById('syndicate-type-badge').textContent = user.syndicate?.type || 'syndicate';
        ...
        document.addEventListener('DOMContentLoaded', function () {
            document.getElementById('sidebar-toggle')?.addEventListener('click', function () {
                document.getElementById('syndicate-sidebar').classList.remove('-translate-x-full');
                document.getElementById('sidebar-backdrop').classList.remove('hidden');
            });
        });
        function closeSidebar() {
            document.getElementById('syndicate-sidebar').classList.add('-translate-x-full');
