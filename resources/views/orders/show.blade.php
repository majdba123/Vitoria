        ...
        <div id="order-content" class="hidden space-y-6">
            <div class="card overflow-hidden">
                <div class="card-body border-b border-gray-100 dark:border-gray-800">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div class="min-w-0">
                            <h1 id="order-number" class="text-xl font-bold text-gray-950 dark:text-white">—</h1>
                        ...
                        <div class="flex flex-wrap items-center gap-2 sm:justify-end">
                            <span id="order-status" class="badge">pending</span>
                            <span id="order-payment" class="badge">cash</span>
                        </div>
                    </div>
                    <div class="mt-3 flex flex-wrap items-center gap-2">
                        <button id="user-cancel-btn" type="button" class="hidden btn-danger btn-xs">Cancel Order</button>
                ...
                <div class="grid gap-3 p-4 sm:grid-cols-2">
                    <div class="border border-gray-100 bg-gray-50 p-3 dark:border-gray-800 dark:bg-gray-800/60" style="border-radius: var(--radius-control)">
                        <p class="text-[11px] font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500">Order ID</p>
                        <p id="order-id" class="mt-1 text-sm font-bold text-gray-900 dark:text-white">—</p>
                    </div>
                    <div class="border border-gray-100 bg-gray-50 p-3 dark:border-gray-800 dark:bg-gray-800/60" style="border-radius: var(--radius-control)">
                        <p class="text-[11px] font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500">Items Count</p>
            ...
            <div class="grid gap-6 lg:grid-cols-12">
                <section class="lg:col-span-8">
                    <div class="card card-body">
                        <div class="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                            <h3 class="text-base font-bold text-gray-900 dark:text-white">Order Items</h3>
                            <span class="badge">Detailed View</span>
                        </div>
                ...
                <aside class="space-y-4 lg:col-span-4">
                    <div class="card card-body">
                        <h3 class="text-sm font-bold text-gray-900 dark:text-white">Totals Summary</h3>
                        <div class="mt-3 space-y-2 text-sm">
                            ...
                            <div class="mt-2 flex items-center justify-between rounded-xl border border-brand-200 bg-brand-50 px-3 py-2 text-base font-bold text-brand-700 dark:border-brand-500/20 dark:bg-brand-500/10 dark:text-brand-300"><span>Total</span><span id="total-val">0 SYP</span></div>
                        </div>
                    </div>
                </aside>
            </div>
...
document.addEventListener('DOMContentLoaded', async function () {
        ...
        itemsEl.innerHTML = (order.items || []).map((item, index) => `
            <article class="card card-body">
                <div class="flex flex-wrap items-start justify-between gap-3">
                    <div>
                        <p class="text-sm font-bold text-gray-900 dark:text-white">${index + 1}. ${esc(item.product_name || 'Product')}</p>
    ...
    function paramCard(label, value) {
        return `<div class="border border-gray-100 bg-gray-50 px-2.5 py-2 dark:border-gray-800 dark:bg-gray-800/60" style="border-radius: var(--radius-control)">
            <p class="text-[10px] font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500">${esc(label)}</p>
