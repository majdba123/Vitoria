...
<div class="space-y-4">
    ...
    <x-alert type="success" id="commission-success" />
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <h2 id="vendor-title" class="mt-1 text-2xl font-bold text-gray-900 dark:text-white">Vendor #{{ $vendorId }}</h2>
                <p class="mt-2 text-sm text-gray-600 dark:text-gray-300">Commission is calculated by category commission for completed orders only.</p>
            </div>
            <a href="/admin/vendors" class="btn-secondary btn-sm">Back to Vendors</a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            <p class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Paid to Vendor</p>
    ...
    <div class="grid grid-cols-1 gap-4 xl:grid-cols-2">
        <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            <h3 class="text-base font-bold text-gray-900 dark:text-white">Order Status Statistics</h3>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            <h3 class="text-base font-bold text-gray-900 dark:text-white">Update Paid Amount</h3>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Set total paid to vendor. Remaining amount updates automatically.</p>
            <form id="paid-form" class="mt-4 space-y-3">
                <div>
                    <label for="paid_amount" class="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Paid Amount</label>
                    <input id="paid_amount" name="paid_amount" type="number" min="0" step="0.01" class="input" placeholder="0.00">
                </div>
                <button id="paid-submit" type="submit" class="btn-primary btn-sm">Save Paid Amount</button>
            </form>
        </div>
    </div>

    <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
        <h3 class="text-base font-bold text-gray-900 dark:text-white">Commission by Category</h3>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
        <h3 class="text-base font-bold text-gray-900 dark:text-white">Last 7 Days Completed Orders</h3>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('paid-form').addEventListener('submit', async function (event) {
        event.preventDefault();
        const submitButton = document.getElementById('paid-submit');
        ...
        try {
            const response = await window.axios.post(`/api/admin/vendors/${vendorId}/commission-paid`, {
                paid_amount: paidAmount,
            });
            latestPaidAmount = Number(response.data?.data?.paid_amount || paidAmount);
            showAlert('commission-success', response.data?.message || 'Paid amount updated.');
            await loadStats();
        } catch (error) {
            const message = error.response?.data?.message || 'Failed to update paid amount.';
            showAlert('commission-alert', message);
        } finally {
            submitButton.disabled = false;
            submitButton.textContent = 'Save Paid Amount';
        }
    });

    async function loadStats() {
        try {
            const response = await window.axios.get(`/api/admin/vendors/${vendorId}/commission-stats`);
            ...
            return `
                <div class="flex flex-col items-center gap-2 rounded-xl border border-gray-200 bg-gray-50 p-2 dark:border-gray-700 dark:bg-gray-800/50">
                    <div class="flex h-24 items-end">
                        <div class="w-5 rounded-t bg-brand-500/90" style="height:${height}px"></div>
                    </div>
