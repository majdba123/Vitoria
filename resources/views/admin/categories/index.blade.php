...
@section('title', 'Categories — SyriaZone Admin')
...
<div class="space-y-4">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <p class="text-sm text-gray-500">Manage product categories and their subcategories.</p>
        </div>
        <a href="{{ route('admin.categories.create') }}" class="btn-primary btn-sm w-full shrink-0 sm:w-auto">
    ...
    {{-- Empty State --}}
    <div id="categories-empty" class="hidden">
        <div class="card py-16 text-center">
            ...
            <h3 class="mt-3 text-sm font-semibold text-gray-900">No categories yet</h3>
            <p class="mt-1 text-sm text-gray-500">Get started by creating a new category.</p>
            <div class="mt-5">
                <a href="{{ route('admin.categories.create') }}" class="btn-primary btn-sm">Add Category</a>
            </div>
        </div>
    </div>

    {{-- Categories Grid --}}
    <div id="categories-grid" class="hidden grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3"></div>
</div>
    ...
    <div class="w-full max-w-sm rounded-xl bg-white p-6 shadow-2xl">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="mt-0.5 text-sm text-gray-500">This will permanently delete the category and all its subcategories.</p>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function loadCategories() {
        try {
            ...
            showAlert('categories-alert', 'Failed to load categories.');
            loading.classList.add('hidden');
        }
    }
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="mt-4 flex gap-2 border-t border-gray-100 pt-4">
                        <a href="/admin/categories/${category.id}" class="btn-primary btn-sm flex-1">View Details</a>
                        <a href="/admin/categories/${category.id}/edit" class="btn-secondary btn-sm">Edit</a>
                        <button onclick="confirmDelete(${category.id})" class="btn-danger btn-sm">Delete</button>
                    </div>
                </div>
            </div>
        `).join('');
    ...
    deleteConfirm.addEventListener('click', async () => {
        ...
        try {
            await window.axios.delete(`/api/admin/categories/${categoryToDelete}`);
            showAlert('categories-success', 'Category deleted successfully.');
            deleteModal.classList.add('hidden');
            categoryToDelete = null;
            loadCategories();
        } catch (e) {
            showAlert('categories-alert', e.response?.data?.message || 'Failed to delete category.');
        }
    });
