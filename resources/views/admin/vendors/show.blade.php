...
@section('title', 'Vendor Details - Vetora Admin')
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    try {
        ...
        document.getElementById('vendor-store-name').textContent = vendor.store_name || '-';
        document.getElementById('vendor-store').textContent = vendor.store_name || '-';
        document.getElementById('vendor-primary-category').innerHTML = renderCategoryBadges(vendor);
        document.getElementById('vendor-address').textContent = vendor.address || 'No address provided.';
        document.getElementById('vendor-city').textContent = vendor.city?.name || 'No city provided.';
        document.getElementById('vendor-description').textContent = vendor.description || 'No description provided.';
        document.getElementById('vendor-created').textContent = vendor.created_at ? new Date(vendor.created_at).toLocaleDateString() : '-';
        document.getElementById('vendor-source').textContent = vendor.registration_source === 'self' ? 'Self registration' : 'Admin';
        ...
        document.getElementById('vendor-phone').textContent = vendor.user?.phone_number || '-';
        document.getElementById('vendor-email').textContent = vendor.user?.email || '-';
        document.getElementById('vendor-national-id').textContent = vendor.user?.national_id || '-';
        ...
        if (vendor.commercial_register_url) {
            document.getElementById('commercial-register-content').innerHTML = `
                <a href="${vendor.commercial_register_url}" target="_blank" rel="noopener" class="btn-secondary btn-sm inline-flex">
        ...
        const categoriesContainer = document.getElementById('vendor-categories');
        if (vendor.categories && vendor.categories.length > 0) {
            categoriesContainer.innerHTML = vendor.categories.map(function (category) {
                return `<span class="inline-flex items-center gap-1.5 rounded-full bg-brand-50 px-3 py-1.5 text-xs font-semibold text-brand-700 ring-1 ring-brand-200">${esc(category.name)} <span class="text-emerald-600">(${parseFloat(category.commission || 0).toFixed(2)}%)</span></span>`;
            }).join('');
        } else {
            categoriesContainer.innerHTML = '<span class="text-sm text-gray-400 italic">No categories assigned</span>';
        }

        document.getElementById('show-loading').classList.add('hidden');
        document.getElementById('show-content').classList.remove('hidden');
    } catch (error) {
        document.getElementById('show-loading').innerHTML = `<p class="text-red-500">${esc(parseBackendError(error, 'Failed to load vendor.'))}</p>`;
    }
    ...
    document.getElementById('approve-btn').addEventListener('click', async function () {
        ...
        try {
            ...
            showAlert('vendor-show-alert', parseBackendError(error, 'Failed to approve vendor.'));
        }
    });
    ...
    function renderCategoryBadges(vendor) {
        ...
        return vendor.categories.map(function (category) {
            return `<span class="badge badge-info">${esc(category.name)}</span>`;
        }).join(' ');
    }
