@extends('layouts.app')
@section('title', 'Ø¬Ù…ÙŠØ¹ Ø§Ù„Ù…Ù†ØªØ¬Ø§Øª - Vetora')
...
<div class="bg-transparent">
    <div class="border-b border-white/40 bg-white/60 backdrop-blur-xl dark:border-white/10 dark:bg-white/5">
        <div class="page-shell py-3">
            <nav class="page-breadcrumb">
                ...
                <span class="font-medium text-gray-900 dark:text-white">Ø¬Ù…ÙŠØ¹ Ø§Ù„Ù…Ù†ØªØ¬Ø§Øª</span>
            </nav>
        </div>
    </div>
    ...
    <div class="page-shell">
        <div class="page-header">
            <div>
                <h1 class="text-2xl font-black text-gray-900 sm:text-3xl dark:text-white">Ø¬Ù…ÙŠØ¹ Ø§Ù„Ù…Ù†ØªØ¬Ø§Øª</h1>
            ...
            <div class="filter-panel grid w-full gap-3 md:grid-cols-2 xl:grid-cols-[repeat(3,minmax(0,1fr))_auto_auto] xl:items-center">
                <select id="f-category-type" class="form-select min-w-0">
                    ...
                    <option value="agriculture">Ø²Ø±Ø§Ø¹ÙŠ</option>
                    <option value="veterinary">Ø¨ÙŠØ·Ø±ÙŠ</option>
                </select>
                <select id="f-category" class="form-select min-w-0"><option value="">ÙƒÙ„ Ø§Ù„ØªØµÙ†ÙŠÙØ§Øª</option></select>
                <select id="f-discount" class="form-select min-w-0">
                    <option value="">ÙƒÙ„ Ø§Ù„Ø®ØµÙˆÙ…Ø§Øª</option>
                    <option value="1">Ø¹Ù„ÙŠÙ‡ Ø®ØµÙ…</option>
                    <option value="0">Ø¨Ø¯ÙˆÙ† Ø®ØµÙ…</option>
                </select>
                <button id="btn-apply" class="btn-primary w-full sm:w-auto">ØªØ·Ø¨ÙŠÙ‚</button>
                <button id="btn-clear" class="btn-secondary w-full sm:w-auto">Ù…Ø³Ø­</button>
            </div>
        </div>
        ...
        <div id="empty" class="empty-state hidden py-20">
            ...
            <p class="mt-4 font-bold text-gray-600 dark:text-gray-400" id="empty-message">Ù„Ø§ ØªÙˆØ¬Ø¯ Ù…Ù†ØªØ¬Ø§Øª</p>
        </div>
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    async function init() {
        try {
            ...
            showEmpty('ØªØ¹Ø°Ø± ØªØ­Ù…ÙŠÙ„ Ø§Ù„Ù…Ù†ØªØ¬Ø§Øª. ÙŠØ±Ø¬Ù‰ Ø§Ù„Ù…Ø­Ø§ÙˆÙ„Ø© Ù…Ø±Ø© Ø£Ø®Ø±Ù‰.');
        }
    }

    function populateCategories() {
        const cats = ct ? allCats.filter(c => c.type === ct) : allCats;
        $('f-category').innerHTML = '<option value="">ÙƒÙ„ Ø§Ù„ØªØµÙ†ÙŠÙØ§Øª</option>' + cats.map(c => `<option value="${c.id}">${esc(c.name)}</option>`).join('');
    }
    ...
    async function load() {
        ...
        try {
            ...
            $('result-count').textContent = `${meta.total} Ù…Ù†ØªØ¬`;
            if (!data.length) {
                showEmpty('Ù„Ø§ ØªÙˆØ¬Ø¯ Ù…Ù†ØªØ¬Ø§Øª Ù…Ø·Ø§Ø¨Ù‚Ø©.');
            } else {
    ...
    function productCard(p) {
        ...
        const typeLabel = p.category?.type === 'agriculture' ? 'Ø²Ø±Ø§Ø¹ÙŠ' : (p.category?.type === 'veterinary' ? 'Ø¨ÙŠØ·Ø±ÙŠ' : '');
        ...
        return `<div class="product-card overflow-hidden rounded-2xl border border-gray-200/80 bg-white dark:border-gray-800 dark:bg-gray-900">
            <div class="relative">
                <a href="/products/${p.id}"><div class="relative aspect-[4/5] overflow-hidden bg-gray-50 dark:bg-gray-800">
                    ...
                    ${!inStock ? '<div class="absolute inset-0 flex items-center justify-center bg-white/70 dark:bg-gray-900/70"><span class="rounded-full bg-red-100 px-3 py-1 text-[11px] font-bold text-red-600 dark:bg-red-500/10 dark:text-red-400">Ù†ÙØ¯ Ù…Ù† Ø§Ù„Ù…Ø®Ø²ÙˆÙ†</span></div>' : ''}
            ...
            <div class="p-3 sm:p-4">
                ...
                <div class="mt-1.5 flex items-center gap-1.5 text-amber-400">${starStars(p.average_rating)}<span class="text-[11px] text-gray-400 dark:text-gray-500">${revCount ? revCount + ' ØªÙ‚ÙŠÙŠÙ…' : ''}</span></div>
                ...
                <button onclick="window.addToCart&&window.addToCart(${p.id},'${esc(p.name)}',${p.has_active_discount ? p.discounted_price : p.price},'${esc(photo)}')" class="mt-3 flex w-full items-center justify-center gap-2 rounded-xl py-2.5 text-xs font-bold ${inStock ? 'bg-gray-900 text-white hover:bg-brand-600 dark:bg-white dark:text-gray-900 dark:hover:bg-brand-500 dark:hover:text-white active:scale-[.97]' : 'bg-gray-100 text-gray-400 cursor-not-allowed dark:bg-gray-800 dark:text-gray-600'}" ${!inStock ? 'disabled' : ''}>
                    ...
                    ${inStock ? 'Ø£Ø¶Ù Ø¥Ù„Ù‰ Ø§Ù„Ø³Ù„Ø©' : 'Ù†ÙØ¯ Ù…Ù† Ø§Ù„Ù…Ø®Ø²ÙˆÙ†'}
                </button>
            </div>
        </div>`;
    }
    ...
    function renderPag(meta) {
        ...
        let html = `<button onclick="goP(${current - 1})" class="flex h-10 items-center gap-1.5 rounded-xl border border-gray-200 bg-white px-4 text-xs font-bold text-gray-600 shadow-sm dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 ${current === 1 ? 'pointer-events-none opacity-40' : ''}" ${current === 1 ? 'disabled' : ''}>Ø§Ù„Ø³Ø§Ø¨Ù‚</button>`;
        ...
        html += `<button onclick="goP(${current + 1})" class="flex h-10 items-center gap-1.5 rounded-xl border border-gray-200 bg-white px-4 text-xs font-bold text-gray-600 shadow-sm dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 ${current === last ? 'pointer-events-none opacity-40' : ''}" ${current === last ? 'disabled' : ''}>Ø§Ù„ØªØ§Ù„ÙŠ</button>`;
