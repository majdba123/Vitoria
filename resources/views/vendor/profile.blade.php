...
@section('title', 'My Profile — Vetora Vendor')
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function loadProfile() {
        try {
            ...
            if (user.avatar_url) {
                setCircleImage('avatar-display', user.avatar_url, i18n.avatarAlt);
            }

            if (vendor?.logo_url) {
                setCircleImage('store-logo-display', vendor.logo_url, i18n.logoAlt);
            }
    ...
    function setCircleImage(containerId, url, alt) {
        document.getElementById(containerId).innerHTML = `<img src="${esc(url)}" alt="${esc(alt || '')}" class="h-full w-full object-cover">`;
    }
    ...
    document.getElementById('logo-input').addEventListener('change', function (e) {
        ...
        if (logoFile) {
            const reader = new FileReader();
            reader.onload = ev => setCircleImage('store-logo-display', ev.target.result, 'Store logo preview');
    ...
    form.addEventListener('submit', async function (e) {
        ...
        try {
            ...
            if (vendorAvatar) {
                if (user.avatar_url) {
                    vendorAvatar.innerHTML = `<img src="${esc(user.avatar_url)}" alt="" class="h-full w-full rounded-full object-cover">`;
                } else {
