...
@section('title', 'Product Reviews — SyriaZone Vendor')
@section('page-title', 'Product Reviews')

@section('content')
<div class="mx-auto max-w-5xl">
    <nav class="mb-4 flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
        <a href="{{ route('vendor.products.index') }}" class="hover:text-gray-700 dark:hover:text-gray-300">Products</a>
    ...
    <div class="card overflow-hidden border border-gray-200 shadow-sm dark:border-gray-800 dark:bg-gray-900">
        ...
        @if($reviews->isEmpty())
            <div class="px-4 py-14 text-center">
                ...
                <a href="{{ route('vendor.products.index') }}" class="btn-secondary btn-sm mt-4 inline-block">Back to Products</a>
            </div>
        @else
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        @foreach($reviews as $review)
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/30">
                                ...
                                <td class="px-4 py-3">
                                    <form action="{{ route('vendor.products.reviews.destroy', [$product->id, $review->id]) }}" method="POST" class="inline" onsubmit="return confirm('Delete this review?');">
                                        @csrf
                                        @method('DELETE')
    ...
    <div class="mt-4 flex gap-2">
        <a href="{{ route('vendor.products.show', $product->id) }}" class="btn-secondary btn-sm">View Product</a>
        <a href="{{ route('vendor.products.index') }}" class="btn-secondary btn-sm">Back to Products</a>
    </div>
</div>
@endsection
