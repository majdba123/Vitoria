...
(function() {
            ...
            return `<div class="group relative overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm transition-shadow hover:shadow-md">
                ...
                <div class="space-y-2 px-2.5 py-2">
                    ...
                    <div>
                        ...
                        <select data-photo-type-index="${i}" class="form-select w-full !py-1 text-xs">
                            <option value="front" ${f._imageType === 'front' ? 'selected' : ''}>Front</option>
                            <option value="back" ${f._imageType === 'back' ? 'selected' : ''}>Back</option>
                        </select>
                    </div>
                    <div>
                        <label class="mb-1 block text-[10px] font-medium uppercase tracking-wide text-gray-500">Order</label>
                        <input type="number" min="1" value="${i + 1}" data-photo-sort-index="${i}" class="form-input !py-1 text-xs">
                    </div>
                </div>
            </div>`;
        }).join('');
    ...
    window.getSelectedPhotoPayload = function () {
        return selectedFiles.map((file, index) => {
            const sortInput = preview.querySelector(`[data-photo-sort-index="${index}"]`);

            return {
                file,
                image_type: file._imageType || 'front',
                sort_order: Number.parseInt(sortInput?.value || `${index + 1}`, 10) || index + 1,
            };
        });
    };
