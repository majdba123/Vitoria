...
@section('page-title', __('vendor.notifications_title'))

@section('content')
<div class="space-y-4">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('vendor.notifications_copy') }}</p>
    ...
    <div id="vendor-notif-empty" class="hidden card py-14 text-center">
        <p class="text-sm font-medium text-gray-600 dark:text-gray-300">{{ __('nav.no_notifications') }}</p>
    </div>
    ...
    <div id="vendor-notif-list-wrap" class="hidden overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
        ...
        <div id="vendor-notif-pagination" class="flex items-center justify-between border-t border-gray-100 px-4 py-3 dark:border-gray-800">
            ...
            <div class="flex gap-2">
                <button type="button" id="vendor-notif-prev" class="btn-secondary btn-xs">{{ __('nav.prev') }}</button>
                <button type="button" id="vendor-notif-next" class="btn-secondary btn-xs">{{ __('nav.next') }}</button>
            </div>
        </div>
    </div>
</div>
...
(function () {
    ...
    const i18n = {!! json_encode([
        'markRead' => __('nav.mark_notification_read'),
        'page' => __('nav.page'),
        'of' => __('nav.of'),
    ...
    function load() {
        ...
        window.axios.get('/api/notifications', { params: { page: state.page, per_page: 15 } }).then(function (res) {
            ...
            document.getElementById('vendor-notif-page-info').textContent = i18n.page + ' ' + currentPage + ' ' + i18n.of + ' ' + lastPage + (total ? ' (' + total + ')' : '');
            ...
            document.getElementById('vendor-notif-loading').classList.add('hidden');
            document.getElementById('vendor-notif-empty').classList.remove('hidden');
            document.getElementById('vendor-notif-empty').querySelector('p').textContent = i18n.failedToLoad;
        });
    }
