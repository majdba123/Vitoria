...
<head>
    ...
    <style>
        ...
        .meta td { padding:5px 12px 5px 0; vertical-align:top; font-size:10px; }
...
<table class="meta"><tr><td><span class="lbl">{{ $l['period'] }}</span>{{ $period['from'] ?? $l['not_available'] }} — {{ $period['to'] ?? $l['not_available'] }}</td><td><span class="lbl">{{ $l['generated'] }}</span>{{ $data['generated_at']->timezone(config('app.timezone'))->format('Y-m-d H:i') }}</td></tr></table>
<table class="kpis"><tr><td><span class="lbl">{{ $l['vendors'] }}</span><strong>{{ $data['kpis']['vendors'] }}</strong></td><td><span class="lbl">{{ $l['active'] }}</span><strong>{{ $data['kpis']['active_vendors'] }}</strong></td><td><span class="lbl">{{ $l['orders'] }}</span><strong>{{ $data['kpis']['completed_orders'] }}</strong></td></tr><tr><td><span class="lbl">{{ $l['units'] }}</span><strong>{{ $data['kpis']['units_sold'] }}</strong></td><td><span class="lbl">{{ $l['sales'] }}</span><strong>{{ $money($data['kpis']['gross_sales']) }}</strong></td><td><span class="lbl">{{ $l['refunds'] }}</span><strong class="negative">{{ $money($data['kpis']['refunds']) }}</strong></td></tr></table>
