...
export function DashboardVendorMap({ endpoint, adminDrilldown = false }) {
    ...
    const accessibleLabel = (region) => {
        ...
        if (payload.domain) return `${name}، ${scopedLabel}: ${region?.vendor_count ?? 0}`;
        return `${name}، ${agricultureLabel}: ${region?.agriculture_count ?? 0}، ${veterinaryLabel}: ${region?.veterinary_count ?? 0}`;
    };
                ...
                {status === 'ready' && (
                    <div ref={containerRef} className="relative isolate mx-auto w-full max-w-3xl overflow-hidden rounded-lg bg-black p-2 sm:p-4">
                        <div className="relative aspect-[512/468] w-full">
                            <img src="/images/syria-governorates-map.jpg" alt="" className="absolute inset-0 size-full object-contain" draggable="false" />
                            <svg viewBox="0 0 512 468" role="group" aria-label={title} className="absolute inset-0 size-full">
