...
export default function CategoriesShow({ categoryId }) {
    ...
    return (
        <AdminLayout title={category?.name ?? admin.categories}>
            <PageHeader
                breadcrumb={[{ label: admin.categories, href: route('admin.categories.index') }, { label: common.view_details ?? 'Details' }]}
                title={
            ...
            {status === 'error' && <p className="text-sm font-medium text-[var(--color-danger-strong)]">Failed to load category.</p>}

            <DetailCard
                isLoading={status === 'loading'}
                columns={2}
                fields={[
                    { label: admin.type_label, value: category?.type },
                    { label: admin.category_commission, value: category ? `${category.commission || 0}%` : null },
                    { label: admin.products, value: category?.products_count ?? 0 },
                    { label: common.created_at ?? 'Created', value: category ? new Date(category.created_at).toLocaleDateString() : null },
                ]}
            />
        </AdminLayout>
    );
