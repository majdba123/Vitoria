...
const formatDate = (value) => value
    ...
    : '-';
const formatMoney = (value) => `${Number(value || 0).toLocaleString('en-US')} SYP`;
...
async function loadSyndicateDetails() {
    ...
    try {
        ...
        document.getElementById('edit-link').href = '/admin/syndicates/' + syndicate.id + '/edit';
        ...
        card.innerHTML = `
            <div class="flex flex-col gap-5 lg:flex-row lg:items-start">
                <div class="flex h-24 w-24 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-brand-50 text-2xl font-black text-brand-700 ring-1 ring-brand-100">
                    <img src="${esc(syndicate.logo_url)}" class="h-full w-full object-cover" alt="${esc(syndicate.name)}" onerror="this.src='{{ asset('images/syndicate-placeholder.svg') }}'">
                </div>
                <div class="min-w-0 flex-1">
                    <div class="flex flex-wrap items-center gap-2">
                        <h2 class="text-2xl font-black text-gray-900 dark:text-white">${esc(syndicate.name)}</h2>
                        <span class="badge ${syndicate.is_active ? 'badge-success' : 'badge-danger'}">${esc(syndicate.status_label || (syndicate.is_active ? 'Active' : 'Inactive'))}</span>
                        <span class="badge badge-brand">${esc(syndicate.type_label || syndicate.type)}</span>
                    </div>
                    <p class="mt-2 text-sm text-gray-500">${esc(userEmail)} · ${esc(userPhone)}</p>
                </div>
            </div>
        ...
        card.innerHTML = `<p class="py-12 text-center text-sm text-red-500">${esc(parsed.generalMessage || 'Failed to load syndicate details.')}</p>`;
    }
}
...
function infoBlock(label, value) {
    return `
        <div>
            <p class="text-[11px] font-bold uppercase tracking-wide text-gray-400">${esc(label)}</p>
